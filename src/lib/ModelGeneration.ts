...
function generateModels(
    ...
    const entityTemplatePath = path.resolve(templateDir, "entity.mst");
...
function createIndexFile(
    ...
    const templatePath = path.resolve(templateDir, "index.mst");
...
function createTsConfigFile(outputPath: string): void {
    const templatePath = path.resolve(templateDir, "tsconfig.mst");
    const template = fs.readFileSync(templatePath, "UTF-8");
    const compliedTemplate = Handlebars.compile(template, {
        noEscape: true
    });
...
function createTypeOrmConfig(
    ...
    const templatePath = path.resolve(templateDir, "ormconfig.mst");
    const template = fs.readFileSync(templatePath, "UTF-8");
    const compliedTemplate = Handlebars.compile(template, {
        noEscape: true
    });
