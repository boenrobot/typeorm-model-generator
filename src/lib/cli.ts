...
async function CliLogic() {
    ...
    if (process.argv.length > 2) {
        options = await checkYargsParameters(options);
    } else if (!TOMLConfig.fullConfigFile) {
        options = await useInquirer(options);
    }
    options = validateConfig(options);
    const driver = await createDriver(options.connectionOptions.databaseType);
    console.log(
...
async function checkYargsParameters(options: options): Promise<options> {
